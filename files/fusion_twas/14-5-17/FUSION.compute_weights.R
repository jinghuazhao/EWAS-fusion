...
option_list = list(
  ...
  make_option("--PATH_plink", action="store", default="/genetics/bin/plink2 --threads 1", type='character',
              help="Path to plink executable [%default]"),
  make_option("--PATH_gcta", action="store", default="/genetics/bin/gcta_nr_robust", type='character',
              help="Path to plink executable [%default]"),
  make_option("--PATH_gemma", action="store", default="/genetics/bin/gemma_1_thread", type='character',
              help="Path to plink executable [%default]"),
...
arg = paste( opt$PATH_gcta ," --grm ",opt$tmp," --pheno ",pheno.file," --out ",opt$tmp," --reml --reml-no-constrain --reml-lrt 1 --thread-num 1",sep='')
system(arg , ignore.stdout=SYS_PRINT,ignore.stderr=SYS_PRINT)
